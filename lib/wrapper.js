...
class XPDB {
    ...
    get(key, options = {}) {
        ...
        return new Promise((resolve, reject) => {
            this._db.get(key.toString(), (err, result) => {
                if (err) {
    ...
    put(key, value) {
        ...
        return new Promise((resolve, reject) => {
            this._db.put(key, value, err => err ? reject(err) : resolve());
        });
    }
     ...
     */
    keys() {
        return streams.streamToPromise(this._db.createKeyStream());
    }
     ...
     */
    values() {
        return streams.streamToPromise(this._db.createValueStream());
     ...
     */
    entries() {
        return streams.streamToPromise(this._db.createReadStream());
    }
}
